...
const Responses = ({
  ...
  useEffect(() => {
    setIsLoading(true);
    getResponses(formId).then(res => res.json()).then(res => {
  ...
  return <View style={styles.p10}>
      {isLoading && <Loader />}
      <SectionList sections={responseList} renderItem={({
      item
    }) => <Response res={item} />} keyExtractor={item => item.id} renderSectionHeader={({
      section: {
        title
      }
